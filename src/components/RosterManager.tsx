    ...
    <div id="roster-manager-container" className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div 
          ...
          className="group relative bg-white border border-slate-200 hover:border-indigo-400 p-5 rounded-2xl shadow-xs hover:shadow-md transition-all cursor-pointer flex flex-col justify-between"
        >
          <input 
            type="file" 
            ref={fileInputRef} 
        ...
        <div 
          ...
          className={`group relative bg-white border p-5 rounded-2xl shadow-xs hover:shadow-md transition-all cursor-pointer flex flex-col justify-between ${
            activeTab === 'import-paste' ? 'border-indigo-600 ring-2 ring-indigo-100' : 'border-slate-200 hover:border-indigo-400'
          }`}
        >
          <div>
          ...
          <div className="mt-4 flex items-center text-xs font-semibold text-sky-600 gap-1">
            <span>開啟文字輸入框</span>
            <span aria-hidden="true">&rarr;</span>
          </div>
        </div>
        ...
        >
          <div>
            ...
            <h3 className="font-semibold text-slate-800 text-base">模擬名單體驗區</h3>
            <p className="text-slate-500 text-xs mt-1 leading-relaxed">
              提供中小學班級（28人）、研討小班（12人）及<strong>重複姓名測試名單</strong>
            </p>
          </div>
        ...
        <div id="simulated-roster-panel" className="bg-white border border-amber-200 rounded-3xl p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-5">
            {SIMULATED_ROSTER_PRESETS.map((preset) => (
              <div 
                key={preset.id}
                className="border border-slate-200 hover:border-amber-400 hover:shadow-sm rounded-2xl p-4 bg-slate-50/50 flex flex-col justify-between transition-all"
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold px-2 py-0.5 bg-amber-100 text-amber-800 rounded-lg">
                      {preset.count} 人
                    </span>
                ...
                <button
                  ...
                  className="mt-4 w-full py-2 bg-white hover:bg-amber-500 hover:text-white border border-slate-300 hover:border-amber-500 text-slate-700 text-xs font-bold rounded-xl shadow-2xs transition-colors flex items-center justify-center gap-1"
                >
      ...
      {csvPreview && activeTab === 'import-csv' && (
        <div id="csv-preview-panel" className="bg-white border border-indigo-200 rounded-3xl p-6 shadow-sm">
          ...
          {csvDuplicateNames.size > 0 ? (
            <div id="csv-duplicate-banner" className="p-3.5 bg-amber-50 border border-amber-200 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              <div className="flex items-start sm:items-center gap-2 text-amber-900 text-xs">
                <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5 sm:mt-0" />
                <div>
                  <span className="font-bold">在姓名欄位發現重複姓名：</span>
                  ...
                  <p className="text-[11px] text-amber-700 mt-0.5">預覽表格中已標記黃色重複標籤</p>
                </div>
              </div>
              <button
                id="btn-remove-csv-duplicates"
                onClick={handleRemoveDuplicatesFromCSV}
                className="px-3.5 py-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-bold shrink-0 shadow-xs flex items-center gap-1.5 transition-colors self-start sm:self-center"
              >
          ...
          <div className="mt-2">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs text-slate-500">預覽清單（顯示前 8 筆）：</p>
              ...
              <table className="w-full text-xs text-left">
                <thead className="bg-slate-100 text-slate-700 uppercase font-semibold sticky top-0">
                  <tr>
                    ...
                    <th className="p-2.5 text-center w-24">狀態</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 text-slate-600">
                  {csvPreview.rows.slice(0, 8).map((row, rIdx) => {
                    const rowName = row[csvPreview.nameColIdx]?.trim();
                    const isDup = rowName && csvDuplicateNames.has(rowName);
                    return (
                      <tr key={rIdx} className={`hover:bg-slate-50 ${isDup ? 'bg-amber-50/60' : ''}`}>
                        <td className="p-2.5 text-center text-slate-400">{rIdx + 1}</td>
                        {row.map((cell, cIdx) => (
                          <td key={cIdx} className={`p-2.5 ${cIdx === csvPreview.nameColIdx ? 'font-medium text-slate-900' : ''}`}>
                            {cell}
                          </td>
                        ))}
      ...
      {activeTab === 'import-paste' && (
        <div id="paste-names-panel" className="bg-white border border-sky-200 rounded-3xl p-6 shadow-sm">
          ...
          {pasteDuplicateNames.size > 0 && (
            <div id="paste-duplicate-banner" className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-2 text-xs text-amber-900">
                <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0" />
                <span>
                  貼入文字中發現重複姓名：<strong className="text-amber-950">{Array.from(pasteDuplicateNames).join('、')}</strong>
                </span>
              </div>
              <button
                ...
                {parsedPasteStudents.map((std, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isDup && (
                        <span className="text-[9px] bg-amber-600 text-white px-1 py-0.2 rounded font-bold">
                          重複
                        </span>
                      )}
                    </span>
